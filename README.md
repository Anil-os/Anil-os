<div align="center">

# Anil Kumar H

### Web Developer • UI/UX Designer • Data Analyst • Cloud & DevOps

Designing experiences • Developing solutions • Discovering insights

</div>

---

## Who Am I

I am an MCA student passionate about building digital products that combine **beautiful design, smart development, and data-driven thinking**. I enjoy turning ideas into real applications with a strong focus on usability and performance.

---

## My Focus Areas

* Creating modern and responsive **web applications**
* Designing intuitive **UI/UX experiences**
* Exploring **data analytics & AI** for decision making
* Learning **DevOps workflows** and deployment
* Building projects that solve real problems

---

## Skill Universe

### Web & Development

HTML • CSS • JavaScript • Next.js • Tailwind • Responsive UI

### UI/UX Design

Figma • Wireframing • Prototyping • Design Systems • User Research

### Data & AI

Python • Pandas • NumPy • Matplotlib • Seaborn • Machine Learning

### DevOps & Tools

Git • GitHub • Linux Basics • Deployment Concepts

### Visualization

Power BI • Tableau • Excel

---

## What I’ve Built

### Dora AI Copilot

An AI-powered interface built with Next.js and Tailwind focusing on modular architecture and smooth user experience.

### Beast Drive – Organization Website

A responsive website crafted with modern UI/UX principles and clean frontend engineering.

### Sales Data Analytics – iStudio

Worked on cleaning datasets, performing EDA, and building visual reports to support business growth strategies.

---

## Internship Journey

**Deloitte – Data Analytics Virtual Internship (Forage)**

* Analyzed datasets for patterns and risks
* Created structured business insights
* Followed professional analytics standards

**iStudio – Sales Analytics Internship**

* Data visualization & KPI reporting
* Business-focused analytics
* UI-oriented dashboard thinking

---

## Certifications Snapshot

GenAI – BCGX • AI Foundations – Oracle • Cybersecurity – Deloitte & Tata •
Software Engineering – Accenture • Cloud Architecture – AWS • Google Cloud – Simplilearn

---

## Let’s Connect

* GitHub: [https://github.com/Anil-os](https://github.com/Anil-os)
* Email: [anil.kumar.devv@gmail.com](mailto:anil.kumar.devv@gmail.com)

---

<div align="center">

### “Creativity in Design • Logic in Code • Insight in Data”

</div>
